...
def breadth_first_graph_search(problem,callback):
    ...
    closed = {}    
    bFringe = FIFOQueue()
    bFringe.append(Node(problem.initial))
    while bFringe:
        node = bFringe.pop()
        if problem.goal_test(node.state):
            callback(problem.graph,node,bFringe,closed,True)
            return node
        if node.state not in closed:
            closed[node.state] = True
            listOfNodes = node.expand(problem)
            nodePathCost = []
            for i in listOfNodes:
                nodePathCost.append((i, i.path_cost - node.path_cost))
            nodePathCost.sort(key = lambda x: x[1])
            orderedNodes = []
            for j in nodePathCost:
                orderedNodes.append(j[0])
            bFringe.extend(orderedNodes)
            callback(problem.graph,node,bFringe,closed,False)
    return None
...
def depth_first_graph_search(problem,callback):
    ...
    closed = {}    
    dFringe = Stack()
    dFringe.append(Node(problem.initial))
    while dFringe:
        node = dFringe.pop()
        if problem.goal_test(node.state):
            callback(problem.graph,node,dFringe,closed,True)
            return node
        if node.state not in closed:
            closed[node.state] = True
            listOfNodes = node.expand(problem)
            ...
            dFringe.extend(orderedNodes)
            callback(problem.graph,node,dFringe,closed,False)
    return None
...
def depth_limited_search(problem, limit, callback):
    ...
    while dFringe:
        ...
        if node.state not in closed:
            ...
            callback(problem.graph,node,dFringe,closed,False)
        if node.depth < limit:
            dFringe.extend(node.expand(problem))
...
def iterative_deepening_search(problem,callback):
    """Iterative deepening using depth limited search"""
...
def best_first_graph_search(problem, f, callback):
    ...
    return graph_search(problem, PriorityQueue(min,f), callback)
...
def astar_search(problem, callback, h=None):
    ...
    return best_first_graph_search(problem, f, callback)
