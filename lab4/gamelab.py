...
def alphabeta(state, game):
    ...
    def max_value(state, alpha, beta):
        ...
        value = -infinity
        for (a,s) in game.successors(state):
            value = max(value, min_value(s, alpha, beta))
            if value >= beta:
                return value
    ...
    def min_value(state, alpha, beta):
        ...
        value = infinity
        for (a,s) in game.successors(state):
            value = min(value, max_value(s, beta, alpha))
            if value >= alpha:
                return value
        beta = min(beta , value)
        return value
...
def alphabeta_cutoff(state, game, d=4, eval_fn=None):
    ...
    def min_value(state, alpha, beta, depth):
        ...
        for (a,s) in game.successors(state):
            value = min(value, max_value(s, beta, alpha, depth +1))
            if value <= alpha:
             return value
...
def query_player(game, state):
    ...
    return literal_eval(raw_input('Your move? '))
...
class TicTacToe(Game):
    ...
    def k_in_row(self, board, move, player, delta):
        ...
        line.remove(move)
        if len(line) >= self.k: return line
        else: return []
...
class GameInterface:
    def __init__(self,game,title="Default",geometry="750x500"):
        ...
        self.alphaBetaButton = Radiobutton(self.controlFrame1, text="AlphaBeta", variable=self.mode, value=3).pack(anchor=W)
        self.alphaBetaCutoffButton = Radiobutton(self.controlFrame1, text="AlphaBeta with cutoff", variable=self.mode, value=4).pack(anchor=W)
